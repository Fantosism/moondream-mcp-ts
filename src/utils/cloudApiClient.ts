...
export class CloudApiClient {
  ...
  async initialize(): Promise<void> {
    ...
    if (!this.config.apiKey) {
      throw new Error('API key is required for cloud mode. Set MOONDREAM_API_KEY environment variable.');
    }
    
    if (!this.config.apiEndpoint) {
      throw new Error('API endpoint is required for cloud mode. Set MOONDREAM_API_ENDPOINT environment variable.');
    }

    // Test API connectivity
    try {
      await this.testConnection();
      this.logger.info('Cloud API connection established', {
        endpoint: this.config.apiEndpoint
      }, 'cloud-api');
    } catch (error) {
      this.logger.error('Failed to connect to cloud API', {
  ...
  async caption(imageBuffer: ImageBuffer, length: 'short' | 'normal' | 'detailed'): Promise<{ caption: string; confidence: number }> {
    ...
    try {
      ...
      this.logger.performance('cloud-caption-inference', inferenceTime, {
        length,
      ...
      this.logger.error('Cloud caption inference failed', {
        error: error instanceof Error ? error.message : String(error)
      }, 'cloud-api');
      
      throw new InferenceError(`Cloud caption inference failed: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  async query(imageBuffer: ImageBuffer, question: string): Promise<{ answer: string; confidence: number }> {
    ...
    try {
      ...
      const inferenceTime = Date.now() - startTime;
      this.logger.performance('cloud-query-inference', inferenceTime, {
      ...
      this.logger.error('Cloud query inference failed', {
        error: error instanceof Error ? error.message : String(error),
        question
      }, 'cloud-api');
      
      throw new InferenceError(`Cloud query inference failed: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  async detect(imageBuffer: ImageBuffer, objectName: string): Promise<{ objects: any[] }> {
    ...
    try {
      ...
      const inferenceTime = Date.now() - startTime;
      this.logger.performance('cloud-detection-inference', inferenceTime, {
        objectName,
      ...
      this.logger.error('Cloud detection inference failed', {
        error: error instanceof Error ? error.message : String(error),
        objectName
      }, 'cloud-api');
      
      throw new InferenceError(`Cloud detection inference failed: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  async point(imageBuffer: ImageBuffer, objectName: string): Promise<{ points: any[] }> {
    ...
    try {
      ...
      const inferenceTime = Date.now() - startTime;
      this.logger.performance('cloud-pointing-inference', inferenceTime, {
        objectName,
      ...
      this.logger.error('Cloud pointing inference failed', {
        error: error instanceof Error ? error.message : String(error),
        objectName
      }, 'cloud-api');
      
      throw new InferenceError(`Cloud pointing inference failed: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
